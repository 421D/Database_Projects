  ...
  <head>
	...
	<script type="text/javascript">
		...
		function cktj(){
			...
			var sqls="select tmp.*,zdname as eaddr,tname,traintype from (select piao.*,zdname as saddr from piao,zhandian where piao.szdid=zhandian.id and SUBSTRING(yxsj,1,10)='"+sj+"' and zdname like '%"+saddr+"%') tmp,zhandian,traininfo where tmp.tid=traininfo.id and tmp.ezdid=zhandian.id and zdname like '%"+eaddr+"%' and tmp.id not in(select pid from dpiao)";
			$("#sql").val(sqls);
			$("#ckf").submit();
		}
		function dpbt(pid){
