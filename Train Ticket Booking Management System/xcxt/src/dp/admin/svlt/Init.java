...
public class Init extends HttpServlet {
	...
	public void doPost(HttpServletRequest request, HttpServletResponse response)
		...
		if("piao".equals(tname)){
			...
			DbConn db3=new DbConn();
			ResultSet rs3=db3.query("select * from zhandian");
			List<Zhandian> zdlist=FindService.getZhandian(rs3);
			request.setAttribute("zdlist", zdlist);
			
			DbConn db4=new DbConn();
			ResultSet rs4=db4.query("SELECT DISTINCT stime FROM piao");
			List<Piao> piaoStimelist=FindService.getPiaoStime(rs4);
			request.setAttribute("piaoStimelist", piaoStimelist);
