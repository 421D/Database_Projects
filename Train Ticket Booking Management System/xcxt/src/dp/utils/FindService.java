...
public class FindService {
		...
		if(rs!=null){
			try {
				...
				while(rs.next()){
					...
					a.setYxsj(yx);
					a.setReservationcode(rs.getString("reservationcode"));
					alist.add(a);
				}
			} catch (SQLException e) {
				// TODO Auto-generated catch block
		...
		if(rs!=null){
			try {
				...
				while(rs.next()){
					...
					try {
						a.setPiaonum(rs.getString("piaonum") == null ? "" : rs
								.getString("piaonum"));
						a.setBuynum(rs.getString("buynum") == null ? "" : rs
								.getString("buynum"));
						a.setSurplus(rs.getString("surplus") == null ? "" : rs
								.getString("surplus"));
					} catch (Exception e) {
						a.setPiaonum("");
						a.setBuynum("");
	...
	//将结果集转化成链表
		public static List<Piao> getPiaoStime(ResultSet rs){
			List<Piao> alist=new ArrayList();
			if(rs!=null){
				try {
					rs.beforeFirst();
